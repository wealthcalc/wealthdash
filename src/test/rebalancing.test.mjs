...
import { allocationDrift, sellSuggestions, buySuggestions, rebalancePlan } from "../core/rebalancing.mjs";
...
test("allocationDrift: computes current vs target weight and drift value", () => {
  const positions = [
    pos({ ticker: "EQ1", kind: "equity", marketValue: 8000 }),
    pos({ ticker: "BF1", kind: "bond_fund", marketValue: 2000 }),
  ];
  const d = allocationDrift({ positions, targets: { equity: 60, bond_fund: 40 } });
  assert.equal(d.total, 10000);
  const eq = d.rows.find((r) => r.kind === "equity");
  const bf = d.rows.find((r) => r.kind === "bond_fund");
  assert.equal(eq.currentPct, 80);
  assert.equal(eq.targetPct, 60);
  assert.equal(eq.driftValue, 2000); // overweight by £2000
  assert.equal(bf.currentPct, 20);
  assert.equal(bf.driftValue, -2000); // underweight by £2000
  assert.equal(d.targetsSumTo100, true);
});

test("allocationDrift: a kind with no target is treated as 0% target, not ignored", () => {
  const positions = [pos({ ticker: "CASH1", kind: "cash", marketValue: 5000 })];
  const d = allocationDrift({ positions, targets: {} });
  const cash = d.rows.find((r) => r.kind === "cash");
  assert.equal(cash.targetPct, 0);
  assert.equal(cash.driftValue, 5000); // fully overweight — flagged to trim
});

test("allocationDrift: flags when targets don't sum to 100", () => {
  const d = allocationDrift({ positions: [pos({ marketValue: 100 })], targets: { equity: 50 } });
...
test("allocationDrift: unpriced positions are excluded from the total", () => {
  ...
  const d = allocationDrift({ positions, targets: { equity: 100 } });
...
test("sellSuggestions: sheltered wrapper holdings are sold first, tax-free", () => {
  ...
  const drift = allocationDrift({ positions, targets: { equity: 0 } }); // fully overweight, sell everything
...
test("sellSuggestions: CGT-exempt gilts rank alongside sheltered wrappers", () => {
  const positions = [
    ...
    pos({ ticker: "EQ1", wrapper: "GIA", kind: "gilt", cgtExempt: false, marketValue: 2000, unrealisedPct: 0.05 }),
  ];
  const drift = allocationDrift({ positions, targets: { gilt: 0 } });
...
test("sellSuggestions: losses are sold before gains even with zero AEA left", () => {
  ...
  const drift = allocationDrift({ positions, targets: { equity: 0 } });
  const s = sellSuggestions({ positions, driftRows: drift.rows, aeaLeft: 0 });
...
test("sellSuggestions: gain realised is pro-rata to the fraction of the pool sold (Section 104)", () => {
  ...
  const drift = allocationDrift({ positions, targets: { equity: 50 } }); // total=2000, target 50% -> targetValue 1000, drift 1000
  const s = sellSuggestions({ positions, driftRows: drift.rows, aeaLeft: 10000 });
...
test("sellSuggestions: AEA budget is shared across kinds, not per-kind", () => {
  const positions = [
  ...
  const drift = allocationDrift({ positions, targets: { equity: 0, bond_fund: 0 } });
  const s = sellSuggestions({ positions, driftRows: drift.rows, aeaLeft: 1000 });
  // total need = 6000, only 1000 of AEA to go around both kinds
...
test("sellSuggestions: nothing overweight -> no suggestions", () => {
  ...
  const drift = allocationDrift({ positions, targets: { equity: 100 } });
  const s = sellSuggestions({ positions, driftRows: drift.rows, aeaLeft: 3000 });
...
test("buySuggestions: lists existing holdings of an underweight kind, largest first, never invents a new fund", () => {
  const positions = [
  ...
  const drift = allocationDrift({ positions, targets: { equity: 60, bond_fund: 40 } });
  const b = buySuggestions({ positions, driftRows: drift.rows });
  assert.equal(b.length, 1);
  assert.equal(b[0].kind, "bond_fund");
  assert.equal(b[0].existingHoldings[0].ticker, "BIG_BOND");
  assert.equal(b[0].existingHoldings[1].ticker, "SMALL_BOND");
});

test("buySuggestions: an underweight kind with no existing holdings still gets a row (empty holdings list)", () => {
  const positions = [pos({ marketValue: 1000 })];
  const drift = allocationDrift({ positions, targets: { equity: 50, cash: 50 } });
  const b = buySuggestions({ positions, driftRows: drift.rows });
  const cashRow = b.find((r) => r.kind === "cash");
  assert.ok(cashRow);
  assert.deepEqual(cashRow.existingHoldings, []);
});
...
test("rebalancePlan: combines drift, sells and buys into one result", () => {
  ...
  const plan = rebalancePlan({ positions, targets: { equity: 50, bond_fund: 50 }, aeaLeft: 3000 });
  assert.ok(plan.rows.length === 2);
  assert.equal(plan.sells.rows[0].ticker, "EQ1"); // overweight equity, sheltered, sells first
  assert.equal(plan.buys[0].kind, "bond_fund");
});

...
test("an index-linked gilt missing its indexation data is shown but refused, not silently hidden", () => {
  // Index-linked gilts ARE supported (see gilts-index-linked.test.mjs) —
  // but only with the DMO's own lag and index ratio. This BODY row carries
  // neither, so it's offered with the reason rather than dropped. Refusing
  // on an unknown lag is deliberate: an 8-month linker needs a different
  // formula, and "probably 3" isn't good enough to value money on.
  const il = shapeGiltCatalogue(BODY).rows.find((r) => r.indexLinked);
  assert.equal(il.supported, false);
  assert.match(il.unsupportedReason, /indexation lag/);

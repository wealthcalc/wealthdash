...
test("shapeFlexPull: empty pull produces a helpful warning instead of silently returning nothing", () => {
  const r = shapeFlexPull({ trades: [], cashTransactions: [] });
  assert.equal(r.trades.length, 0);
  assert.equal(r.income.length, 0);
  assert.ok(r.warnings.some((w) => /no Trade or Cash Transaction rows/i.test(w)));
});

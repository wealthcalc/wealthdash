    ...
    value: ok
      ? {
        ...
        patch: {
          ...
          isin: isin || upper(secMeta[ticker]?.isin) || "",
        },
      }
      : null,
  };
...
export function shapeGiltCatalogue(body = {}) {
  ...
  for (const [isin, d] of Object.entries(body.prices || {})) {
    ...
    rows.push({
      ...
      supported: !d.indexLinked,
      unsupportedReason: d.indexLinked ? "Index-linked — this app models fixed coupons and par redemption only, so its figures would be wrong." : null,
    });
  }

...
- **New: "Income by month" chart** on the Tax by year sub-tab, alongside the existing year-on-year wrapper chart — same stacked-by-wrapper shape, but at calendar-month granularity (last 24 months) so quarterly/annual dividend clustering is visible rather than smoothed into one tax-year bar.
...
## Tests
```
npm test        # node --test: 368 tests across the core modules + the DMO parser
```
